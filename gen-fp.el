...
(defun fp-qf-pad (attrs num-1)
  (let* ((num (- num-1 1))
         (shape (or (plist-get attrs :shape) 'rect))
...
(defun fp-plot-courtyard (max-x max-y &optional space)
  (let* ((space (or space 0.25))
         (x (+ max-x space))
         (y (+ max-y space)))
    `((fp_line (start ,(- x) ,(- y))
               (end ,(- x) ,y)
               (layer Cmts.User) (width 0.05))
      (fp_line (start ,(- x) ,y)
               (end ,x ,y)
               (layer Cmts.User) (width 0.05))
      (fp_line (start ,x ,y)
...
(defun fp-qf-header (attrs)
  (let* ((font-data '((layer F.SilkS)
                      (effects (font (size 0.81 0.81) (thickness 0.15)))))
